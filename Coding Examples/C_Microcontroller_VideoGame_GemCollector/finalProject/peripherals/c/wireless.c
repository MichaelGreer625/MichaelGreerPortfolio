...
static __INLINE void wireless_set_tx_addr(uint8_t  *tx_addr)
{
	...
	uint8_t i;
	num_bytes = 6;
	tx_data[0] = NRF24L01_TX_ADDR_R|NRF24L01_CMD_W_REGISTER;
	for(i = 1; i < num_bytes; i++)
		tx_data[i] = tx_addr[i-1];
	
	wireless_CSN_low();
	spiTx(RF_SPI_BASE, tx_data, num_bytes, rx_data);
	wireless_CSN_high();
}
...
static __INLINE void wireless_tx_data_payload( uint32_t data)
{
	...
	for(i = 1; i < num_bytes; i++)
		tx_data[i] = (data >> (8*(i-1))) & 0x000000FF;
