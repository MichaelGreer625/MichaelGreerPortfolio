...
)
{
  ...
  if( (image_height_pixels & 0x01) == 0x00)
  {
	...
	//shifting if subtracted too far
	if(x0 > 240){
		x1 = x1 - x0;
		x0 = 0;
	}
        ...
        for(j= 0; j < image_width_bits; j++)
        {
            if( (j %8) == 0)
            {
              ...
              data1 = image1[byte_index];
							data2 = image2[byte_index];
            }
            if ( data1 & 0x80)
            {
                lcd_write_data_u16(fColor1);
            }
						else if ( data2 & 0x80)
						{
                lcd_write_data_u16(fColor2);
            }
            else
            {
                lcd_write_data_u16(bColor);
...
void lcd_draw_color_image(uint16_t image_width,uint16_t row_number,uint8_t *image_data){
	int i, j;
	uint16_t rgb_color;
	...
	//centering
	 x0 = 120 - image_width/3/2;
	 x1 = 120 + image_width/3/2;
	
	//setting position
	lcd_set_pos(x0, x1, row_number, row_number);
	
	for(i = 0; i < image_width; i+= 3){
		rgb_color = 0;
		red = image_data[i]>>3;				//grabbing only msb values corresponding to each color
		green = image_data[i+1]>>2;
		blue = image_data[i+2]>>3;
		red = red<<11;								//getting bits in right position of total rgb bits
		green = green<<5;
		rgb_color = (red | green | blue);		//combining colors into one bits
		lcd_write_data_u16(rgb_color);			//writing the combined color
	}		
...
void lcd_draw_rectangle_centered(uint16_t x_center, uint16_t x_len, uint16_t y_center, uint16_t y_len,uint16_t fg_color){
	//for loop indecies
	int i;
	int j;

	//calculating and setting rectangle corners (funky calculations is for correct behavior for both odd and even)
	uint16_t x0 = x_center - x_len/2;
	uint16_t x1 = x_center + x_len/2 - 1 + x_len%2;
	uint16_t y0 = y_center - y_len/2;
	uint16_t y1 = y_center + y_len/2 - 1 + x_len%2;
	
	//shifting if subtraction went too far
	if(x0 > 240){
		x1 = x1 - x0;
		x0 = 0;
	}
